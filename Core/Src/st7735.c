...
/*
void ST7735_HLine(uint16_t x, uint16_t x2, uint16_t y, uint16_t color)
{
	uint16_t i;
	uint8_t CH = color >> 8;
	uint8_t CL = (uint8_t)color;

	if((x >= ST7735_WIDTH) || (x2 >= ST7735_WIDTH) || (y >= ST7735_HEIGHT))
		return;

	ST7735_Select();
	if (x < x2) ST7735_SetAddressWindow(x, y, x2, y);
	else ST7735_SetAddressWindow(x2, y, x, y);
//set pin DC
	HAL_GPIO_WritePin(ST7735_DC_GPIO_Port , ST7735_DC_Pin , 1 );

	for(i = (x2 - x); i > 0; i--)
	{
		ST7735_Write_Byte(CH);
		ST7735_Write_Byte(CL);
	}
	ST7735_Unselect();
}
*/
